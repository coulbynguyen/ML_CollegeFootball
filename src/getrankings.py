...
def get_team_ranking(week,away,home):
    raw_training = []
    ...
    away_features = []
    home_features = []
    away_features.append(int(team_ranks[away]))
    home_features.append(int(team_ranks[home]))
    ...
    for feature in features_list:
        for x,y in enumerate(feature):
            if away == y[0]:
                away_features.append(x+1)
            if home == y[0]:
                home_features.append(x+1)

    away_features.append(games_played[away])
    home_features.append(games_played[home])
    away_features += home_features
